...
export function createAiStreamPretextRenderer(container) {
    if (!(container instanceof HTMLElement)) {
        throw new Error('AI stream container is required');
    }

    container.replaceChildren();

    const stage = document.createElement('div');
    stage.className = 'ai-pretext-stage';
    ...
    function syncLinePool(nextLength) {
        while (lineNodes.length < nextLength) {
            ...
            lineNode.className = 'ai-pretext-line';
    ...
    function renderNow() {
        ...
        const lineHeight = parsePixels(
            style.lineHeight,
            Math.round(parsePixels(style.fontSize, 14) * 1.6)
        );
