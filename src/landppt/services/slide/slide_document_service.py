...
class SlideDocumentService:
    ...
    def _generate_fallback_slide_html(self, slide_data: Dict[str, Any], page_number: int, total_pages: int) -> str:
        ...
        if slide_type == 'title':
            ...
            points = content_points or ['下一章节']
            points_html = ''.join([f"<div style='font-size: clamp(1rem, 2vw, 1.3rem); color: rgba(255,255,255,0.82); margin-top: 10px;'>{point}</div>" for point in points[:2]])
            content_html = f"""
                <div style="width: 100%; aspect-ratio: 16/9; box-sizing: border-box; margin: 0 auto; position: relative; max-width: 1200px; height: 100%; display: flex; align-items: center; justify-content: center; overflow: hidden; background: linear-gradient(135deg, #111827 0%, #0f766e 100%); color: white;">
                    <div style="position: absolute; inset: 0; opacity: 0.18; background-image: linear-gradient(90deg, rgba(255,255,255,.25) 1px, transparent 1px), linear-gradient(rgba(255,255,255,.25) 1px, transparent 1px); background-size: 64px 64px;"></div>
                    <div style="position: relative; z-index: 2; width: min(900px, 78%);">
                        <div style="font-size: 18px; letter-spacing: .18em; text-transform: uppercase; color: rgba(255,255,255,0.68); margin-bottom: 18px;">Transition</div>
                        <h1 style="font-size: clamp(2.8rem, 7vw, 5rem); line-height: 1.05; margin: 0 0 18px; color: #ffffff;">{title}</h1>
                        <div style="width: 120px; height: 4px; background: #5eead4; border-radius: 999px; margin: 0 0 26px;"></div>
                        {points_html}
                    </div>
                    <div style="position: absolute; bottom: 15px; right: 20px; color: rgba(255,255,255,0.75); font-size: clamp(10px, 1.5vw, 14px);">第{page_number}页 / 共{total_pages}页</div>
                </div>
                """
